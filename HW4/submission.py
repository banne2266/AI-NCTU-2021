...
class ExactInference(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                current_prob = self.belief.getProb(row, col)
                x = util.colToX(col)
                y = util.rowToY(row)
                std = Const.SONAR_STD
                mean = math.sqrt((x - agentX) ** 2 + (y - agentY) ** 2 )
                prob = util.pdf(mean, std, observedDist)
                self.belief.setProb(row, col, prob * current_prob)
    ...
    def elapseTime(self) -> None:
        ...
        for row in range(self.belief.getNumRows()):
            backup.append([])
            for col in range(self.belief.getNumCols()):
                backup[row].append(self.belief.getProb(row, col))
                self.belief.setProb(row, col, 0)
        for row_to in range(self.belief.getNumRows()):
            for col_to in range(self.belief.getNumCols()):
                for row_from in range(self.belief.getNumRows()):
                    for col_from in range(self.belief.getNumCols()):
                        if ((row_from,col_from), (row_to,col_to)) in self.transProb:
                            to_add = backup[row_from][col_from] * self.transProb[((row_from,col_from), (row_to,col_to))]
                            self.belief.addProb(row_to, col_to, to_add)
        self.belief.normalize()
        # END_YOUR_CODE
...
class ParticleFilter(object):
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        prob_distri = collections.defaultdict(float)
        for particle in self.particles:
            x = util.colToX(particle[1])
            y = util.rowToY(particle[0])
            mean = math.sqrt((agentX - x) ** 2 + (agentY - y) ** 2)
            std = std = Const.SONAR_STD
            prob_distri[particle] = self.particles[particle] * util.pdf(mean, std, observedDist)
        new_particle = collections.defaultdict(int)
        for i in range(self.NUM_PARTICLES):
            new_loc = util.weightedRandomChoice(prob_distri)
            new_particle[new_loc]+=1
    ...
    def elapseTime(self) -> None:
        ...
        for particle in self.particles:
            if particle in self.transProbDict:
                cnt = self.particles[particle]
                for i in range(cnt):
                    new_loc = util.weightedRandomChoice(self.transProbDict[particle])
                    new_particle[new_loc]+=1
        self.particles = new_particle
        #print(prob_distri)
