...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        data_num = featureVals.shape[1]
        feature_num = featureVals.shape[0]
        errors = np.zeros(feature_num)
        
        for i in range(feature_num):
            for j in range(data_num):
                h = classifier(featureVals[i][j])
                errors[i] += weights[j] * abs(h - labels[j])
        ...
        for i in range(feature_num):
            if errors[i] < bestError:
                ...
                bestError = errors[i]
        bestClf = WeakClassifier(features[lowest_error_index])   
